...
float NDF_TrowbridgeReitzGGX(float alhpa, float NdotH) {
...
void main() {
    ...
    for(uint i = 0u; i < SAMPLE_COUNT; i++) {
        ...
        float D = NDF_TrowbridgeReitzGGX((roughness*roughness), NdotH);
        float pdf = (D * NdotH / (4.0 * HdotV)) + 0.0001;

        float resolution = cubemapRes;
